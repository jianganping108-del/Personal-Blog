...
<template>
  <div v-if="project" class="detail-page">
    ...
    <section class="detail-content section-shell">
    ...
    <section class="highlight-section"><div class="section-shell highlight-inner"><div><p class="eyebrow">关键实践</p><h2>我在项目中<br />解决的问题。</h2></div><div class="highlight-list"><article v-for="(highlight, index) in project.highlights" :key="highlight"><span>{{ String(index + 1).padStart(2, "0") }}</span><Check :size="18" /><p>{{ highlight }}</p></article></div></div></section>
